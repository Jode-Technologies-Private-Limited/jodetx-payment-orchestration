...
title: Quick Start
excerpt: >-
  Get your first S2S Payment Gateway transaction running in under 10 minutes. 
  Make your first successful test transaction in under 10 minutes by following
  these five steps.
deprecated: false
hidden: false
metadata:
...
const encrypted = encryptPayload(payload, process.env.ENC\_KEY, process.env.ENC\_IV);
...
const result = decryptResponse(response.encryptedData, process.env.ENC\_KEY, process.env.ENC\_IV);
console.log(result);
// {
